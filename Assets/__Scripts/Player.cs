...
using System.Linq; // Enables LINQ querries
...
[System.Serializable] // Make the Player class visible in the Unity Inspector pane
public class Player {
	...
	public CardBartok AddCard(CardBartok eCB){
		...
		if (type == PlayerType.human) {
			...
			// couple of miliseconds), but since we are only doing it once
			// every turn it is not a problem.
		}
	...
	public void FanHand() {
		...
		// Move all the cards to their new position
		Vector3 pos;
		float rot;
		Quaternion rotQ;
		for (int i=0; i<hand.Count; i++) {
			...
			// ^ Quarternion representing the same rotation as rot

			// pos is a V3 half a card height above [0,0,0] (i.e., [0,1.75,0])
			pos = Vector3.up * CardBartok.CARD_HEIGHT/2f;

			// Multiplying a Quarternion by a Vector3 rotates that Vector3 by 
			// the rotation stored in the Quarternion. The result gives us a
			...
			pos += handSlotDef.pos;
			// This staggers the cards in teh Z direction, which is not visible 
			// but which does keep their colliders from overlapping
			pos.z = -0.5f * i;

			// This line below makes sure that the card starts moving immediately
			// if it is not the initial deal at the beginnig of the game.
	...
	public void TakeTurn(){
		...
		// Do not need to do anything if this is the human player.
