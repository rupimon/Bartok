...
//============================ Bézier Curves ============================
	...
	static public Quaternion Bezier( float u, List<Quaternion> vList ) {
		...
		Quaternion res = Quaternion.Slerp( Bezier(u, vListL), Bezier(u, vListR), u ); return( res );
	}
	
	// This version allows an Array or a series of floats as input
	static public Quaternion Bezier( float u, params Quaternion[] vecs ) { return( Bezier( u, new List<Quaternion>(vecs) ) );
	}
