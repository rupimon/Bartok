...
public class Bartok : MonoBehaviour {
	...
	void LayoutGame() {
		...
		for (int i = 0; i<numStartingCards; i++) {
			for (int j = 0; j<4; j++) { // There are always 4 players
				...
				// Stagger the draw time a bit. Rememeber order of operations
				tCB.timeStart = Time.time + drawTimeStagger * (i*4 + j);
				// ^ by setting the timeStart before calling AddCard, we 
				// overwrite the automatic setting of timeStart in 
				// CardBartok.MoveTo().
		...
		// Call Bartok.DrawForstTarget when the hand cards have been drawn
		Invoke ("DrawFirstTarget", drawTimeStagger * (numStartingCards * 4 + 4));
	}

	public void DrawFirstTarget() {
		// Flip up teh first target card from the drawPile
	...
	public CardBartok MoveToTarget(CardBartok tCB) {
		...
		tCB.faceUp = true;
		tCB.SetSortingLayerName ("10"); // Layout.layerName ???????
	...
	// The Drw function will pull a single card from the drawPile and return it
	public CardBartok Draw() {
		CardBartok cd = drawPile [0]; // Pull the 0th card prospector
	...
	*/
	...
	public void CardClicked(CardBartok tCB) {
		// If it is not the human's turn, do not respond
		if (CURRENT_PLAYER.type != PlayerType.human) return;
		// If the game is waiting on a card to move, do not respond
	...
	public bool CheckGameOver() {
		// See if we need to reshuffle the discardPile into the drawPile
		if (drawPile.Count == 0) {
